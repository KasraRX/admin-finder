...
print('my github: https://github.com/KasraRX')
print("welcome to admin finder")

print(Fore.MAGENTA + 'for exzample: https://www.exzample,com/')
url1= input(' enter your url: ')
fix_url(url1)
print(Fore.MAGENTA + 'for exzample: https://www.exzample,com/login.php')
url_exam= input('enter your exam url: ')
fix_url(url_exam)
req_exam =requests.post(url_exam,allow_redirects=False).text
progress=1
stale=570
print(Fore.YELLOW+"program started")

for i in strong:
    try:
        mian_req = requests.get(url1 + i,allow_redirects=False)
        source_req = mian_req.text
        if mian_req.status_code == 200 and source_req != req_exam:
            print('\a')
            print(Fore.WHITE+'['+Fore.BLUE+'+'+Fore.WHITE+'] '+Fore.GREEN+str(url1+i)+Fore.LIGHTMAGENTA_EX+"===>"+Fore.WHITE+"("+"progress"+str(progress)+')'+Fore.LIGHTMAGENTA_EX+"____"+Fore.WHITE+"("+"stale"+str(stale)+')')
        else:
            print(Fore.WHITE+'['+Fore.RED+'-'+Fore.WHITE+'] '+Fore.RED+str(url1+i)+Fore.LIGHTMAGENTA_EX+"===>"+Fore.WHITE+"("+"progress"+str(progress)+')'+Fore.LIGHTMAGENTA_EX+"____"+Fore.WHITE+"("+"stale"+str(stale)+')')
        progress+=1
        stale-=1
    except: print('server lost')
print('\a')
print(Fore.YELLOW+"program end")